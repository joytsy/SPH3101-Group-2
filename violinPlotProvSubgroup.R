...
if(1)
{
  ...
  grid.xaxis(label= rep(c("Baseline", "Follow-Up"), length(xlevels)/2), # varying depending on number of labels
             at = 1:8,gp=gpar(fontsize=8)) ## change at according to number of levels
  ...
  grid.text('Distribution of Stigma Scores across Provinces', y=unit(1, 'npc')+unit(1, 'lines'), gp=gpar(fontsize=10,fontface = 'bold')) ## change title
